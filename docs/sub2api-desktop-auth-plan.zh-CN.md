...
支付处理中 -> 在当前标签页进入支付服务并等待服务端履约
支付成功 -> 同一标签页自动返回原授权会话
已订阅 -> 自动创建设备 Key 并完成授权
```

桌面授权购买会保留当前浏览器标签，不使用支付弹窗；普通网页充值不带桌面授权回跳地址时仍沿用原有弹窗交互。桌面端轮询兑换成功后会恢复、显示并聚焦主窗口，避免依赖浏览器页面调用 `window.close()`。
...
- [x] “订阅服务”选项的默认和未连接状态。
- [x] Mac 授权页面打开、同窗口支付回跳和 App 自动完成授权。
