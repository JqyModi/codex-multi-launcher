...
![选择订阅套餐](landing/subscription-assets/06-choose-plan.png)

- 页面显示“已售罄”时，该档试运营名额已经用完，请选择仍可购买的套餐。
- 同一套餐可能设置单用户限购；达到上限后无需重复购买，可等待续期或选择其他套餐。

- `支付宝`：人民币结算。
- `微信支付`：人民币结算。
- `国际银行卡`：美元结算，支持页面显示的银行卡及 Link 等方式。

![选择支付方式](landing/subscription-assets/07-payment-methods.png)
...
> 新生成的隔离 Profile 第一次打开官方 Codex / ChatGPT App 时，官方 App 可能先显示工作类型等首次使用引导。选择适合的选项或点击 `Skip` 完成一次即可；这不是订阅失效，也不需要改为 ChatGPT 账号登录。完成后会进入当前 Profile 的 Codex 界面。
...
![重新授权现有订阅 Profile](landing/subscription-assets/08-reauthorize-profile.png)
...
### 打开后出现 ChatGPT 的工作类型引导

这是官方 Codex / ChatGPT App 为新的隔离用户目录显示的一次性首次使用页面。选择任意适合的工作类型或点击 `Skip`，完成后即可进入 Codex；无需重新购买、重新授权或填写 API Key。
